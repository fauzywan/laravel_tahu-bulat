...
document.addEventListener('click', function (e) {
    ...
    if (e.target.classList[2] == "remove") {
        ...
        jumlah = e.target.parentNode.parentNode.parentNode.querySelector('.total-input').value

        if (document.getElementById('bayars_field').value.length > 3) {
            if (jumlah.length > 3) {
                jumlah = jumlah.split(',').join('');
            }
            jumlah = parseInt(document.getElementById('bayars_field').value.split(',').join('')) - parseInt(jumlah);
        }
        if (jumlah.length > 3) {
            jumlah = jumlah.toString().match(/\d{1,3}/g);

            if (jumlah[jumlah.length - 1].length == 1) {
                jumlah = jumlah.join('').substr(0, 1) + "," + jumlah.join('').substr(1).match(/\d{1,3}/g)
            }

            if (jumlah[jumlah.length - 1].length == 2) {
                jumlah = jumlah.join('').substr(0, 2) + "," + jumlah.join('').substr(2).match(/\d{1,3}/g)


            }
            if (jumlah[jumlah.length - 1].length == 3) {
                jumlah = jumlah.join('').substr(0, 3) + "," + jumlah.join('').substr(3).match(/\d{1,3}/g)
    ...
    if (e.target.classList[0] == "remove-tr") {
        jumlah = e.target.parentNode.parentNode.querySelector('.total-input').value
    ...
    if (e.target.classList[2] == "lunas") {
        e.target.parentNode.parentNode.parentNode.querySelector('.dibayar-input').value = e.target.parentNode.parentNode.parentNode.querySelector('.total-input').value.toString()
    }
    if (e.target.classList[0] == "lunas-tr") {
        e.target.parentNode.parentNode.querySelector('.dibayar-input').value = e.target.parentNode.parentNode.querySelector('.total-input').value.toString()
    }
...
function hs() {
    [...document.querySelectorAll('.harga-input')].map(harga => {
        harga.addEventListener('keyup', function () {
            ...
            if (parent_let.querySelector('.kuantitas-input').value.length > 0 || this.value > 0) {
                uang = this.value * parent_let.querySelector('.kuantitas-input').value;

                if (this.value.length > 3) {
                    uang = this.value.split(',').join('') * parent_let.querySelector('.kuantitas-input').value;
                }
                uang = uang.toString().split(',').join('').match(/\d{1,3}/g);

                if (this.value.length > 0) {
                    if (uang.join('').length > 3) {
                        if (parent_let.querySelector('.kuantitas-input').value > 0) {

                            if (uang[uang.length - 1].length == 1) {
                                uang = uang.join('').substr(0, 1) + "," + uang.join('').substr(1).match(/\d{1,3}/g);
                            }

                            if (uang[uang.length - 1].length == 2) {
                                uang = uang.join('').substr(0, 2) + "," + uang.join('').substr(2).match(/\d{1,3}/g);


                            }
                            if (uang[uang.length - 1].length == 3) {
                                uang = uang.join('').substr(0, 3) + "," + uang.join('').substr(3).match(/\d{1,3}/g);
            ...
            if (this.value.length > 0) {
                ...
                if (parent_let.querySelector('.harga-input').value.length > 0) {
                    ...
                    if (parent_let.querySelector('.harga-input').value.length > 3) {
                        uang = parent_let.querySelector('.harga-input').value.split(',').join('') * this.value

                        uang = uang.toString().split(',').join('').match(/\d{1,3}/g);

                        if (uang[uang.length - 1].length == 1) {
                            parent_let.querySelector('.total-input').value = uang.join('').substr(0, 1) + "," + uang.join('').substr(1).match(/\d{1,3}/g);
                        }

                        if (uang[uang.length - 1].length == 2) {
                            parent_let.querySelector('.total-input').value = uang.join('').substr(0, 2) + "," + uang.join('').substr(2).match(/\d{1,3}/g);


                        }
                        if (uang[uang.length - 1].length == 3) {
                            parent_let.querySelector('.total-input').value = uang.join('').substr(0, 3) + "," + uang.join('').substr(3).match(/\d{1,3}/g);
